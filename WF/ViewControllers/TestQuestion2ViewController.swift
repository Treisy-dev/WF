...
class TestQuestion2ViewController: UIViewController {
    ...
    private func setUpAnwerButtons(){
        ...
        createButton(answer1, "\tA.\t" + "Stones", 302)
        createButton(answer2, "\tB.\t" + "Food", 376)
        createButton(answer3, "\tC.\t" + "Jokes", 450)
    }
    ...
    private func timerAction(){
        ...
        if timernumber == -4{
            timer.invalidate()
            if answer2.backgroundColor == .appOrangeBlink {
                corectAnswers += 1
            }
            let vc = FinishTrainingViewController(corectAnswer: corectAnswers, questionCount: questionCount)
